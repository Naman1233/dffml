...
class TestFileSource(AsyncTestCase):
                ...
                "source": {
                    ...
                    "config": {
                        "file": {
                            "arg": None,
                            "config": {
                                "filename": {"arg": Arg(type=str), "config": {},},
                                "readwrite": {
                                    "arg": Arg(
    ...
    def test_config_readonly_default(self):
        config = FileSource.config(parse_unknown("--source-file-filename", "feedface"))
    ...
    def config(self, filename, tag="untagged", readwrite=True, allowempty=True):
        return FileSourceConfig(
            filename=filename, readwrite=readwrite, tag=tag, allowempty=allowempty,
        )

    async def test_open(self):
        m_open = mock_open()
        with patch("os.path.exists", return_value=True), patch("builtins.open", m_open):
            async with FakeFileSource(self.config("testfile", readwrite=False)):
                m_open.assert_called_once_with("testfile", "r")

    async def test_open_gz(self):
        source = FakeFileSource("testfile.gz")
        m_open = mock_open()
        with patch("os.path.exists", return_value=True), patch("gzip.open", m_open):
            async with FakeFileSource(self.config("testfile.gz", readwrite=False)):
                m_open.assert_called_once_with("testfile.gz", "rt")

    async def test_open_bz2(self):
        m_open = mock_open()
        with patch("os.path.exists", return_value=True), patch("bz2.open", m_open):
            async with FakeFileSource(self.config("testfile.bz2", readwrite=False)):
                m_open.assert_called_once_with("testfile.bz2", "rt")

    async def test_open_lzma(self):
        m_open = mock_open()
        with patch("os.path.exists", return_value=True), patch("lzma.open", m_open):
            async with FakeFileSource(self.config("testfile.lzma", readwrite=False)):
                m_open.assert_called_once_with("testfile.lzma", "rt")

    async def test_open_xz(self):
        m_open = mock_open()
        with patch("os.path.exists", return_value=True), patch("lzma.open", m_open):
            async with FakeFileSource(self.config("testfile.xz", readwrite=False)):
    ...
    async def test_close_gz(self):
        m_open = mock_open()
        with patch("os.path.exists", return_value=False), patch("gzip.open", m_open):
    ...
    async def test_close_bz2(self):
        m_open = mock_open()
        with patch("os.path.exists", return_value=False), patch("bz2.open", m_open):
    ...
    async def test_close_lzma(self):
        m_open = mock_open()
        with patch("os.path.exists", return_value=False), patch("lzma.open", m_open):
